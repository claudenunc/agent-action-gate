...
It is designed as a small human-in-the-loop layer for agent runtime safety.
...
Run the blocked exfiltration demo end-to-end:

```bash
python examples/demo_blocked_exfiltration.py
```

The demo creates an isolated runtime, sends a dangerous email proposal through the gate, expects it to be blocked, has the human reject it, verifies the SHA-256 approval chain, and prints the audit view.
...
You can wire this into LangGraph, CrewAI, AutoGen, or any custom Python agent — the gate doesn't care how the proposal was generated, only that it was proposed.
